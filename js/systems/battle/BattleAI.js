...
export class BattleAI {
    ...
    async runEnemyTurn() {
        ...
        // 1. 혼란 상태 (CONFUSION): 80% 무작위 이동 후 타격 / 20% 대기
        if (controlState === 'CONFUSION') {
            ...
            if (this.battle.reachableHexes && this.battle.reachableHexes.length > 0) {
                ...
                if (validMoves.length > 0) {
                    ...
                    this.battle.log(`🌀 ${ai.name}이(가) 방향 감각을 잃고 비틀거리며 무작위로 이동합니다.`, 'log-system');
                    await this.battle.moveUnit(ai, randomHex.q, randomHex.r);
                }
            }

            // 2. 이동이 끝난 후, 주변에 공격 가능한 대상이 있는지 확인한다
            ...
            // 3. 사거리 내에 타겟이 있으면 피아 구분 없이 무작위 공격
            if (targetsInRange.length > 0) {
                ...
                this.battle.log(`💥 ${ai.name}이(가) 눈에 뵈는 게 없이 가까이 있는 ${randomTarget.name}(${targetTypeStr})을(를) 냅다 공격합니다!`, 'log-cc');
                await new Promise(r => setTimeout(r, 300));
                await this.battle.skillProcessor.performAttack(ai, randomTarget, 1.0, "혼란 공격", false, ai.atkType || 'PHYS', 1);
            } else {
                // 사거리 내에 아무도 없으면 그냥 턴 종료
                this.battle.log(`❓ ${ai.name}이(가) 허공에 무기를 휘두릅니다.`, 'log-system');
            }
