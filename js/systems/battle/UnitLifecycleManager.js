...
export class UnitLifecycleManager {
    ...
    initUnits(chapter, stage) {
        ...
        const spawn = (entryData, team, fixedQ = null, fixedR = null) => {
            ...
            // ⭐ [근본 해결 1단계] 세이브 조작 및 오류로 인해 배열에 섞여 들어간 'null' 쓰레기 데이터 원천 정화
            ...
            // ⭐ [버그 수정] equippedSkills는 길이가 6으로 유지되어야 스킬 매니저에서 하드 리셋(초기화)을 당하지 않습니다.
            // null을 완전히 제거해버리면 전투 항복 후 씬 전환 시 스킬이 모두 장착 해제되는 치명적 버그가 발생합니다.
            if (unit.equippedSkills) {
                unit.equippedSkills = unit.equippedSkills.map(id => id === undefined ? null : id);
                while (unit.equippedSkills.length < 6) {
                    unit.equippedSkills.push(null);
                }
                if (unit.equippedSkills.length > 6) {
                    unit.equippedSkills = unit.equippedSkills.slice(0, 6);
                }
            }

            const hasHydratedSkills = unit.skills && unit.skills.length > 0 && typeof unit.skills[0] === 'object' && unit.skills[0].name;            if (!hasHydratedSkills) {
                if (unit.skillIds) {
            ...
            // ⭐ [근본 해결 2단계] 클래스 레벨 강제 조작 시, 누락된 스킬을 역추적하여 영구 복구하는 Catch-up 시스템
            if (unit.team === 0 && typeof SKILL_DATABASE !== 'undefined') {
                const currentClassLv = parseInt(unit.classLevel) || 1;
                if (!unit.skillIds) unit.skillIds = [];
                if (!unit.equippedSkills) unit.equippedSkills = [];

                Object.keys(SKILL_DATABASE).forEach(skillId => {
                    const s = SKILL_DATABASE[skillId];
                    // 현재 직업(job)과 일치하며, 강제로 올린 클래스 레벨 이하의 스킬들을 모두 색출
                    if (s && s.job === unit.job && parseInt(s.req_class_lv || 1) <= currentClassLv) {
                        
                        // 1. 배열에 없는 스킬 강제 습득 (세이브 데이터에 영구 복구됨)
                        if (!unit.skillIds.includes(skillId)) {
                            unit.skillIds.push(skillId);
                            if (!unit.skills.some(obj => obj.id === skillId)) {
                                unit.skills.push(JSON.parse(JSON.stringify({ ...s, id: skillId })));
                            }
                        }
                    }
                });
        ...
        if (!this.battle.isPeaceful) {
            this.battle.units.forEach(u => {
                // ⭐ [버그 수정] 아군(team 0)은 반드시 장착(equippedSkills)한 패시브만 작동하도록 제한
                if (u.skills && u.skills.some(s => s.type === 'PASSIVE' && (u.team !== 0 || (u.equippedSkills && u.equippedSkills.includes(s.id))) && s.effects.some(e => e.type === 'PAS_FIRST_TURN'))) {                    u.actionGauge = this.battle.actionGaugeLimit; 
                ...
                startBuffPassives.forEach(passiveSkill => {
                    // ⭐ [즉사 버그 완벽 차단] PAS_ 로 시작하는 모든 시스템 트리거(예: PAS_ONDEATH)를 확실하게 걸러냅니다.
                    // 이걸 걸러내지 않으면 엔진이 ONDEATH의 'DEATH' 단어를 인식하고 성직자를 즉사시켜 버립니다.
                    ...
                    if (this.battle.skillProcessor && buffEffects.length > 0) {
                        ...
                        buffEffects.forEach(eff => {
                            // 단순 applyStatus가 아닌 정식 processEffect를 통해 타겟(아군 전체 등)에 맞게 정상 분배합니다.
    ...
    spawnUnit(key, team, q, r, overrides = {}, auraEffects = null) {
        ...
        // ⭐ [근본 해결] 강제로 레벨/클래스를 올렸을 때, 조건에 맞는 스킬을 자동 습득 및 장착(Equip) 처리합니다.
        if (unit.team === 0 && unit.classLevel && typeof SKILL_DATABASE !== 'undefined') {
            // 1. 유닛의 직업(job)과 클래스 레벨(classLevel)에 맞는 모든 스킬 강제 습득
            Object.values(SKILL_DATABASE).forEach(s => {
                if (s.job === unit.job && parseInt(s.req_class_lv) <= unit.classLevel) {
                    if (!extractedIds.includes(s.id)) extractedIds.push(s.id);
                }
            });
        }

        // ⭐ [신규 반영] '위압감' 같은 패시브 오라를 소환/초기화 시점에 엔진에 정식 등록
