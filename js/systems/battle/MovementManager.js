...
        const unitMap = new Map(); 
        for (let i = 0; i < this.battle.units.length; i++) { 
            ...
            // ⭐ 전투불능(사망 카운트다운) 유닛도 장애물로 등록
            if (u.curHp > 0 || u.isIncapacitated) unitMap.set(`${u.q},${u.r}`, u); 
        }
...
                // ⭐ [아군 통과 로직 추가] 같은 팀 유닛인지 확인 (단, 석화 상태면 통과 불가)
                const isAlly = uAt && uAt.team === this.battle.currentUnit.team;
                const isPetrified = uAt && this.battle.hasStatus(uAt, 'STAT_PETRIFY');

                if (!uAt || uAt === this.battle.currentUnit || uAt.isNPC || (isAlly && !isPetrified) || (passThrough && uAt.team !== this.battle.currentUnit.team)) {
...
            const uAt = unitMap.get(k);
            
            // ⭐ [도착 불가 처리] 통과는 가능하더라도, 아군이 서 있는 자리를 최종 도착지로 칠하지 않음
            if (!uAt || uAt === this.battle.currentUnit) {
...
        const unitMap = new Map(); 
        ...
        const path = this.battle.grid.findPath({q:unit.q, r:unit.r}, {q, r}, nh => { 
            ...
            // ⭐ 2. 누워있는 시체는 아군/패스스루 여부와 상관없이 무조건 벽으로 취급 (비켜주기 불가)
            if (uAt && uAt.isIncapacitated) return false; 
            
            const isAlly = uAt && uAt.team === unit.team; 
            // ... (이하 원래 있던 패스스루 로직 유지) ...
            const passThrough = (unit.skills || []).some(s => s.type === 'PASSIVE' && s.effects.some(e => e.type === 'PAS_MOVE_THRU'));
            
            return !uAt || uAt === unit || uAt.isNPC || isAlly || (passThrough && uAt.team !== unit.team);
        });
        
...
            // ⭐ [개선: 대기 후 복귀하는 완벽한 통과 애니메이션]
            let passedAlly = null;
            const targetTileUnit = unitMap.get(`${s.q},${s.r}`);
            
            if (targetTileUnit && targetTileUnit !== unit && targetTileUnit.team === unit.team && targetTileUnit.curHp > 0) {
                passedAlly = targetTileUnit;
                
                const scale = this.battle.grid.scale || 1;
                const shiftX = (Math.random() > 0.5 ? 30 : -30) * scale; 
                
                const tData = this.battle.grid.getTerrainData(passedAlly.q, passedAlly.r);
                const basePos = this.battle.grid.hexToPixel3D(passedAlly.q, passedAlly.r, tData.h || 0);

                // 이전 겹침 애니메이션이 남아있다면 강제 종료
                if (passedAlly._dodgeAnim) clearInterval(passedAlly._dodgeAnim);
                
                // 1. 길 비켜주기 (0.1초 만에 빠르게 옆으로 피해서 대기함)
                let startTime = Date.now();
                passedAlly._dodgeAnim = setInterval(() => {
                    let t = (Date.now() - startTime) / 100;
                    if (t > 1) t = 1;
                    passedAlly.visualPos = {
                        x: basePos.x + shiftX * (t * (2 - t)), // 빠르게 감속(Ease-out)
                        y: basePos.y - Math.sin(t * Math.PI) * 10 * scale // 살짝 위로 뜀
                    };
                    if (t === 1) clearInterval(passedAlly._dodgeAnim);
                }, 16);
                
                // 이동하는 유닛은 가볍게 어깨를 틀어주는 반동 효과 유지
                unit.bumpX = -shiftX * 0.5;
                unit.bumpY = -5 * scale;
                
                // 2. 반투명 효과 (Phasing) 적용
                if (!unit.buffs) unit.buffs = [];
                if (!passedAlly.buffs) passedAlly.buffs = [];
                unit.buffs.push({ type: 'TEMP_STEALTH', duration: 1, isPhasingDummy: true });
                passedAlly.buffs.push({ type: 'TEMP_STEALTH', duration: 1, isPhasingDummy: true });
            }
...
            }
            
            // 이동 템포 대기 (비켜준 아군이 이 시간 동안 대기함)
            const moveDelay = this.battle.isPeaceful ? 200 : 350;
            await new Promise(resolve => setTimeout(resolve, moveDelay));
            
            // ⭐ [핵심 추가] 3. 딜레이가 끝나면 투명화를 풀고 스르륵 제자리로 복귀
            if (passedAlly) {
                unit.buffs = unit.buffs.filter(b => !b.isPhasingDummy);
                passedAlly.buffs = passedAlly.buffs.filter(b => !b.isPhasingDummy);

                const tData = this.battle.grid.getTerrainData(passedAlly.q, passedAlly.r);
                const basePos = this.battle.grid.hexToPixel3D(passedAlly.q, passedAlly.r, tData.h || 0);
                const startX = passedAlly.visualPos ? passedAlly.visualPos.x : basePos.x;
                
                if (passedAlly._dodgeAnim) clearInterval(passedAlly._dodgeAnim);
                
                // 0.15초 동안 원래 자리로 자연스럽게 돌아옴
                let returnStart = Date.now();
                passedAlly._dodgeAnim = setInterval(() => {
                    let t = (Date.now() - returnStart) / 150;
                    if (t > 1) t = 1;
                    
                    passedAlly.visualPos = {
                        x: startX + (basePos.x - startX) * t,
                        y: basePos.y
                    };
                    
                    if (t === 1) {
                        clearInterval(passedAlly._dodgeAnim);
                        passedAlly.visualPos = null; // 완전히 복귀하면 렌더러에 위임
                    }
                }, 16);
            }
        }
