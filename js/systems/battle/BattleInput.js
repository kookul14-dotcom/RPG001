...
export class BattleInput {
    ...
    handleMouseMove(e) {
        ...
        // Grid.js 에 구현된 역추적 함수 (pixelToHex)가 알아서 회전 및 높이를 계산해 줍니다!
    ...
    // ==============================================================
    // ⭐ [신규] Q / E 키 입력 처리 (전장 회전 제어)
    // ==============================================================
    rotateMap(direction) {
        if (!this.battle.grid) return;

        // 1. 회전 전, 현재 화면 중앙이 가리키고 있는 논리적 헥스 좌표(q, r)를 기억
        const rect = this.canvas.getBoundingClientRect();
        const centerX = rect.width / 2;
        const centerY = rect.height / 2;
        
        const centerWorldX = centerX + this.battle.camera.x;
        const centerWorldY = centerY + this.battle.camera.y;
        
        // pixelToHex는 현재 카메라 회전 상태가 반영되어 있습니다.
        const centerHex = this.battle.grid.pixelToHex(centerWorldX, centerWorldY);

        // 2. 카메라 각도 증감 (direction: -1(좌), 1(우))
        const currentRot = this.battle.grid.cameraRotation;
        this.battle.grid.setRotation(currentRot + direction);

        // 3. 회전 후, 아까 기억해둔 중앙 헥스(centerHex)가 회전된 화면에서는 
        //    어떤 픽셀 좌표(x, y)에 찍히는지 재계산
        const newCenterPixel = this.battle.grid.hexToPixel3D(centerHex.q, centerHex.r);

        // 4. 화면의 정중앙 픽셀이 그 새로운 픽셀 좌표를 바라보도록 카메라 위치를 강제 이동 (화면 튕김 방지)
        this.battle.camera.x = newCenterPixel.x - centerX;
        this.battle.camera.y = newCenterPixel.y - centerY;

        // 5. 회전했다는 시스템 메세지
        this.battle.log(direction > 0 ? "전장 우회전 (E)" : "전장 좌회전 (Q)", "log-system");
        
        // 플로팅 텍스트나 UI 업데이트
        if (this.battle.ui && this.battle.ui.updateFloatingPosition) {
            this.battle.ui.updateFloatingPosition();
        }
        
        // 마우스 커서 위치를 강제로 한 번 갱신하여 조준 범위를 즉시 렌더링
        this.handleMouseMove({ clientX: centerX + rect.left, clientY: centerY + rect.top, button: -1 }); 
    }
    ...
    handleKeyDown(e) {
        ...
        // ⭐ Q / E 키 입력 시 전장 회전
        if (e.code === 'KeyQ' || e.key === 'q' || e.key === 'ㅂ') {
            e.preventDefault();
            this.rotateMap(-1); // 반시계 방향 (왼쪽)
            return;
        }
        if (e.code === 'KeyE' || e.key === 'e' || e.key === 'ㄷ') {
            e.preventDefault();
            this.rotateMap(1); // 시계 방향 (오른쪽)
            return;
        }
    ...
    handleClick(e) { 
        ...
        if (battle.selectedSkill) { 
            ...
            if (u && battle.grid) {
                ...
                if (dist === 1) {
                    const dir = battle.grid.getDirection({q: u.q, r: u.r}, battle.hoverHex);
                    u.facing = dir;
                }
            }
