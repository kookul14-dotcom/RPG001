...
export class BattleUI {
    constructor(battleSystem, canvas) {
        ...
        // ⭐ 오버레이 컨테이너 초기화 시 이전 잔재 완벽 파괴 보장
        const oldOverlay = document.getElementById('unit-overlays');
        ...
        resetPanelIds.forEach(id => {
            ...
            if (el) {
                ...
                if (id === 'unit-overlays') {
                    ...
                    el.innerHTML = ''; // ⭐ 찌꺼기 방지를 위해 내부 HTML도 싹 비워줍니다.
                }
            }
        });
        ...
        // ⭐ [버그 해결 핵심 1] 재입장 시마다 사이드바 우측 패널 구조를 강제로 클린업하고 재생성합니다.
        if (rightPanel) {
            ...
            if (terrainWrapper) terrainWrapper.remove(); 
            
            const logWrapper = document.createElement('div');
            logWrapper.id = 'battle-log-wrapper';
            logWrapper.className = 'floating-widget';
            // ⭐ [버그 수정] 로그 창을 우측 상단 항복 버튼 바로 밑으로 고정시키고 절대 위치 부여
            logWrapper.style.cssText = 'height: 200px; flex-shrink: 0; transition: opacity 0.3s ease; opacity: 0.8; pointer-events: auto; position: fixed; top: 60px; right: 20px; width: 320px; z-index: 99999;';
            ...
            // ⭐ [버그 수정] 로그창이 타겟 정보창(rightPanel)에 묶여 늘어나거나 가려지는 현상을 막기 위해 바디에 별도 부착
        ...
        this._overlayCache = new Map(); // ⭐ 생성자에서 오버레이 맵 확실하게 초기화
    ...
    initDayNightUI() {
        ...
        if (!filterOverlay) {
            ...
            filterOverlay.style.cssText = 'position: absolute; top: 0; left: 0; width: 100vw; height: 100vh; pointer-events: none; z-index: 50; transition: background 2.5s ease-in-out; background: rgba(0,0,0,0); will-change: background;';
    ...
    _syncUnitBarsAndStatus(u, prefix, isMini = false) {
        ...
        const wtYellowFill = document.getElementById(`bar-${prefix}-wt-fill`);
        const wtRedFill = document.getElementById(`bar-${prefix}-wt-red-fill`);
        const wtText = document.getElementById(`bar-${prefix}-wt-text`);
        
        if (wtYellowFill && wtRedFill && wtText) {
            let ag = Math.floor(u.actionGauge || 0);
            let yellowPct = 0;
            let redPct = 0;

            if (ag >= 0) {
                yellowPct = Math.min(100, (ag / this.battle.actionGaugeLimit) * 100);
            } else {
                redPct = Math.min(100, (Math.abs(ag) / this.battle.actionGaugeLimit) * 100);
            }

            wtYellowFill.style.width = `${yellowPct}%`;
            wtRedFill.style.width = `${redPct}%`;
            wtText.innerText = u.isTerrainMock ? '' : ag;
        }
    ...
    // ⭐ [버그 해결 핵심 2] 오른쪽 패널로 넘어갈 때 발생하던 의문의 중첩 에러를 방지하기 위해 로직을 철저히 캡슐화
    _generateLShapePanel(u, isAlly, titleText = null, prefix = 'left', animClass = '') {
        ...
        if (!document.getElementById('l-shape-width-fix-v16')) {
            ...
            style.innerHTML = `
                ...
                #sidebar-left { left: 20px !important; } 
                /* ⭐ 로그창 영역(상단 300px)을 침범하지 못하도록 우측 패널 최대 높이 제한 */
                #sidebar-right { right: 20px !important; max-height: calc(100vh - 300px) !important; } 
                
                #battle-log-wrapper { position: fixed !important; top: 60px !important; right: 20px !important; width: 320px !important; max-height: 200px !important; z-index: 99999 !important; pointer-events: auto !important; }
        ...
        // ⭐ [버그 해결 핵심] 무한 렌더링을 막기 위해 현재 버프들의 정확한 서명(Signature)을 생성하여 HTML에 주입합니다.
        const currentBuffSig = (u.buffs || []).map(b => b.type + b.duration + (b.amount || '') + (b.val || '')).join(',');

        return `
        <div id="card-${prefix}" style="display:flex; flex-direction:${flexDirection}; align-items:flex-end; filter:drop-shadow(2px 4px 10px rgba(0,0,0,0.8)); pointer-events:auto; width:max-content; flex-wrap:nowrap; ${inlineAnimStyle}">
            ...
            <div style="width:300px; display:flex; flex-direction:column; position:relative; z-index:2; flex-shrink:0;">
                ...
                <div style="width:100%; background:${panelBg}; border:none; border-radius:${portraitBarsRadius}; padding:6px 8px; box-sizing:border-box; display:flex; flex-direction:column; gap:0;">
                    ...
                    <div style="width:100%; height:12px; background:#222; border:1px solid #000; position:relative; border-radius:2px; overflow:hidden;">
                        <div id="bar-${prefix}-wt-fill" style="width:${yellowPct}%; height:100%; background:#ffd700; transition:width 0.2s; position:absolute; left:0; top:0;"></div>
                        <div id="bar-${prefix}-wt-red-fill" style="width:${redPct}%; height:100%; background:#ff4444; transition:width 0.2s; position:absolute; left:0; top:0;"></div>
                        
                        <div id="bar-${prefix}-wt-text" style="position:absolute; width:100%; top:0; line-height:12px; text-align:center; font-size:11px; color:#fff; font-weight:bold; pointer-events:none; text-shadow:1px 1px 0 #818181; font-family:var(--font-game);">
                            ${u.isTerrainMock ? '' : ag}
                        </div>
                    </div>
                </div>
                ...
                <div style="display:flex; justify-content:space-between; align-items:stretch; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:8px;">
                    <div style="display:flex; flex-direction:column; gap:6px; flex:1;">
                        <div style="display:flex; align-items:center; gap:8px;">
                            ...
                            <div id="sig-${prefix}" data-sig="${currentBuffSig}" style="display:flex; flex-wrap:wrap; gap:4px; flex:1;">
    ...
    playTurnEndAnimation() {
        ...
        leftCards.forEach(card => {
            ...
            card.style.setProperty('animation', 'slideOutLeftUI 0.4s cubic-bezier(0.8, 0.2, 0.8, 1) forwards', 'important');
        });
        rightCards.forEach(card => {
            card.style.setProperty('animation', 'slideOutRightUI 0.4s cubic-bezier(0.8, 0.2, 0.8, 1) forwards', 'important');
        });
    ...
    renderPartyList() {
        ...
        if (this._animLeftUnitId !== u.id) {
            // ⭐ [버그 수정] 매 프레임 호출되어 애니메이션 타이머가 계속 리셋되는 현상을 막는 락(Lock)
            if (this._pendingLeftUnitId === u.id) return;

            if (existingCard && !existingCard.classList.contains('sliding-out')) {
                ...
                this._pendingLeftUnitId = u.id; // 예약 잠금
                if (this._renderLeftTimeout) clearTimeout(this._renderLeftTimeout);
                this._renderLeftTimeout = setTimeout(() => {
                    this._pendingLeftUnitId = null; // 잠금 해제
                    this._executeRenderPartyList(u, leftPanel, false); 
                }, 350);
            } else if (existingCard && existingCard.classList.contains('sliding-out')) {
                this._pendingLeftUnitId = u.id; // 예약 잠금
                if (this._renderLeftTimeout) clearTimeout(this._renderLeftTimeout);
                this._renderLeftTimeout = setTimeout(() => {
                    this._pendingLeftUnitId = null; // 잠금 해제
            ...
            if (existingCard && existingCard.classList.contains('sliding-out')) return;
    ...
    updateRightPanel(targets = [], terrain = null) {
        ...
        if (targets.length === 0) {
            ...
            let html = `<div class="${animClass}" style="display:flex; flex-direction:column; max-height:60vh; margin-bottom:10px; width: 240px; background: rgba(26,18,16,0.95); border: 2px solid #5d4037; border-radius: 8px; overflow: hidden; pointer-events: auto; box-shadow: -4px 4px 15px rgba(0,0,0,0.8);">`;
            html += `<div style="background: #c62828; color: #fff; padding:6px; font-size:12px; font-weight:bold; text-align:center; border-bottom: 2px solid #ff5252; letter-spacing:1px;">다중 타겟 (${targets.length}명)</div>`;
            html += `<div class="custom-scrollbar" style="display:flex; flex-direction:column; overflow-y:auto; max-height: 50vh;">`;
            
            targets.forEach((u, idx) => {
                const hpPct = Math.max(0, Math.min(100, (u.curHp / (u.hp || 1)) * 100));
                const portraitSrc = (typeof PORTRAIT_DATA !== 'undefined' && (PORTRAIT_DATA[u.id] || PORTRAIT_DATA[u.key] || PORTRAIT_DATA[u.classKey])) || u.portrait; 
                const iconHtml = portraitSrc ? `<img src="${portraitSrc}" style="width:34px; height:34px; border-radius:4px; object-fit:cover; border:1px solid #000;">` : `<div style="width:34px; height:34px; background:#000; border-radius:4px; display:flex; align-items:center; justify-content:center; font-size:18px;">${u.icon}</div>`;
                const nameColor = u.team === 0 ? '#81c784' : '#e57373';

                html += `
                <div id="card-right-${idx}" style="display:flex; align-items:center; gap:10px; padding:8px 10px; border-bottom:1px dashed rgba(255,255,255,0.15);">
                    ${iconHtml}
                    <div style="flex:1; display:flex; flex-direction:column; gap:4px;">
                        <div style="color:${nameColor}; font-size:12px; font-weight:bold; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${u.name} <span style="color:#888; font-size:10px; font-weight:normal;">Lv.${u.level||1}</span></div>
                        <div style="width:100%; height:8px; background:#222; border-radius:2px; overflow:hidden; position:relative; border:1px solid #000;">
                            <div id="bar-right-${idx}-hp-fill" style="width:${hpPct}%; height:100%; background:linear-gradient(90deg, #8b0000, #b22222);"></div>
                            <div id="bar-right-${idx}-hp-text" style="position:absolute; width:100%; text-align:center; font-size:8px; color:#fff; font-weight:bold; line-height:8px; top:0; text-shadow:1px 1px 0 #000;">${Math.floor(u.curHp)}</div>
                        </div>
                    </div>
                </div>`;
            });
    ...
    updateFloatingControls() {
        ...
        const buildSkillBtn = (s) => {
            ...
            const cdTurns = (u.cooldowns && u.cooldowns[s.id] > 0) ? u.cooldowns[s.id] : 0;
            const hasActionLock = u.buffs && u.buffs.some(b => b.type === 'DEBUFF_ACTION_LOCK');
            
            const isDisabled = isManaLack || hasActed || isMaterialLack || cdTurns > 0 || hasActionLock;
            
            let cls = `menu-btn ${this.battle.selectedSkill?.id === s.id ? 'active' : ''}`;            if (isDisabled) cls += ' disabled';
            const finalCost = Math.floor((s.cost !== undefined ? s.cost : 50) * costRed);
            
            let nameDisplay = cdTurns > 0 ? `<span style="color:#ff5555; font-weight:bold; margin-right:4px;">[${cdTurns}턴]</span>${s.name}` : s.name;
            let tooltipExtra = '';
            if (s.isStolen) {
            ...
            let displayMp = s.mp || 0;
            let mpColor = '#90caf9';
            let mpTextAdd = '';
            
            const isChantCategory = s.category && (s.category.includes('HM') || s.category.includes('RQ') || s.category.includes('DR') || s.category.includes('EP') || s.category.includes('GF'));
            if (isChantCategory) {
                const resonance = (u.skills || []).find(sk => 
                    sk.type === 'PASSIVE' && 
                    sk.effects.some(e => e.type === 'PAS_RANGE_CHANT_EXTEND') &&
                    (u.equippedSkills && u.equippedSkills.includes(sk.id))
                );
                
                if (resonance) {
                    displayMp = Math.floor(displayMp * 1.5); 
                    mpColor = '#ff5252'; 
                    mpTextAdd = ' (범위확장)';
                }
            }

            const tooltipStr = `<div style="color:#ffca28 !important; font-weight:bold; text-shadow:1px 1px 2px rgba(0,0,0,0.8);">${s.name}</div>${tooltipExtra}<div style="color:#d7ccc8 !important; margin-top:6px; line-height:1.4; font-size:12px; opacity:1 !important; text-shadow:1px 1px 2px rgba(0,0,0,0.8);">${finalDesc}</div>${materialTooltipHtml}<div class="cost-info" style="margin-top:8px; border-top:1px dashed #5d4037; padding-top:6px;"><span style="color:${mpColor} !important; font-weight:bold;">MP: ${displayMp}${mpTextAdd}</span><span style="color:#ffcc80 !important; font-weight:bold;">WT: ${finalCost}</span></div>`;

            const encodedTooltip = tooltipStr.replace(/"/g, '&quot;').replace(/'/g, '&#39;');            
            const skillClickAttr = isDisabled ? '' : `onclick="event.stopPropagation(); window.battle.selectSkillFromFloat('${s.id}')"`;            return `<div class="${cls}" ${skillClickAttr} data-tip="${encodedTooltip}" onmouseenter="window.battle.ui.showTooltip(event, this.getAttribute('data-tip'))" onmouseleave="window.battle.ui.hideTooltip()" style="display:flex; flex-direction:column; align-items:flex-start;"><div style="width:100%;"><span>${s.icon||'✨'}</span> ${nameDisplay}</div></div>`;
        };
        ...
        this.battle.units.forEach((u, index) => {
            ...
            if (!div) {
                ...
                div.innerHTML = `
                    ...
                    <div class="dead-sprite-icon" style="display:${(isFullyDead || isIncapacitated) ? 'block' : 'none'}; font-size:26px; font-weight:bold; color:#ffdd00; text-align:center; filter:drop-shadow(1px 2px 3px rgba(0,0,0,1)) drop-shadow(-1px -1px 0 rgba(0,0,0,0.8)); margin-bottom: 5px;">${displayIcon}</div>                    
            ...
            const targetLeft = Math.round(finalX) + 'px';
            const targetTop = Math.round(finalY) + 'px';

            if (div.style.left !== targetLeft) div.style.left = targetLeft;
            if (div.style.top !== targetTop) div.style.top = targetTop;
            ...
            if (div.style.zIndex !== newZIndex) div.style.zIndex = newZIndex;            if (div._deadSpriteIcon) {
            ...
            if (ag >= 0) { 
                yellowWidth = Math.floor(Math.min(100, (ag / this.battle.actionGaugeLimit) * 100)); 
            } else { 
                redWidth = Math.floor(Math.min(100, (Math.abs(ag) / this.battle.actionGaugeLimit) * 100)); 
            }
            ...
            if (div._agFill) {
                div._agFill.style.width = yellowWidth + '%';
                div._agFill.style.background = '#ffca28'; 
            }
            
            if (!div._agRedFill) {
                const agRow = div.querySelector('.ag-row');
                if (agRow) {
                    const redBar = document.createElement('div');
                    redBar.className = 'ag-fill-red'; 
                    Object.assign(redBar.style, {
                        height: '100%', width: '0%', background: '#ff4444',
                        position: 'absolute', left: '0', top: '0', transition: 'width 0.2s'
                    });
                    agRow.appendChild(redBar);
                    div._agRedFill = redBar;
                }
            }
            
            if (div._agRedFill) {
                div._agRedFill.style.width = redWidth + '%';
            }
    ...
    destroy() {
        ...
        // ⭐ 클래스가 메모리에서 파괴될 때도 확실하게 DOM 이중 청소
        this.hideAllCombatUI(); 
    }

    hideAllCombatUI() {
        // ⭐ [버그 픽스] 전투 중 JS가 생성한 모든 동적 스타일시트를 100% 제거하여 CSS 순서 꼬임 원천 차단
        const stylesToRemove = [
            'force-fullscreen-styles', 
            'l-shape-width-fix-v16', 
            'battle-dynamic-styles', 
            'action-ui-styles-v5', 
            'action-ui-styles-v6'
        ];
        ...
        // ⭐ 배틀 로그 래퍼, 타겟 인포 등 이전 스테이지 찌꺼기를 강제 삭제 목록에 추가!
        const removableIds = ['day-night-filter', 'warcraft-clock', 'surrender-btn-container', 'terrain-info-wrapper', 'exit-freeview-btn', 'battle-log-wrapper', 'target-info-wrapper'];
        ...
        hideIds.forEach(id => {
            ...
            if (el) {
                ...
                el.style.pointerEvents = 'none';
                // ⭐ 좌우 UI 찌그러짐 방지 및 DOM 누수 차단
                if (id === 'sidebar-left' || id === 'sidebar-right') {
                    el.style.width = '';
                    el.innerHTML = ''; // 찌꺼기 100% 비우기
                }
            }
        });
        ...
        resultScreens.forEach(screen => {
            if (screen) screen.style.zIndex = '9999999'; 
        });
    }
}
