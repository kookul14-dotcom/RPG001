...
export class StatusManager {
    ...
    applyStatus(target, data, caster) {
        ...
        if (incomingTier > 0) {
            ...
            // (1) 패시브 내성(불굴의 의지) 체크
            const immunePassive = (target.skills || []).find(s => s.type === 'PASSIVE' && s.effects.some(e => e.type === 'STAT_AILMENT_IMMUNE'));
            if (immunePassive) {
        ...
        if (type.startsWith('BUFF_CHANNELED') || type.startsWith('DEBUFF_CHANNELED')) {
            ...
            target.auraEffects = [{ 
                type: auraEffType, val: multiplier, area: data.area !== undefined ? parseInt(data.area) : 999, 
