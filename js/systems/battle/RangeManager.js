...
export class RangeManager {
    ...
    isTargetInValidRange(caster, targetHex, skill) {
        ...
        // 2. 스킬 이름에 돌진기 키워드가 있는지 확인
        const hasDashName = skill.name && ['돌진', '돌격', '혈로', '강철의 행진', '직선'].some(keyword => skill.name.includes(keyword));

        // 3. 직선 범위(LINE)이거나 돌진기라면 무조건 축 정렬 검사 실행
        const isStrictLine = areaStrForLine.includes('LINE') || isDashEffect || hasDashName;

        if (isStrictLine) {
            const dq = targetHex.q - caster.q;
            const dr = targetHex.r - caster.r;
            
            // 육각 그리드 6방향 축 검사 (q축이 같거나, r축이 같거나, 대각선 축이 같아야 함)
            const isOnAxis = (dq === 0) || (dr === 0) || (dq === -dr);
            
            // 축 위에 존재하지 않는 엉뚱한 대각선 타일은 아예 조준 범위에서 제외(false)시킵니다.
            if (!isOnAxis) return false;
        }
        // =======================================================================
