...
export class TownSystem {
    ...
    renderShopItems(stockList, villageKey, shopType) {
        ...
        stockList.forEach((item, idx) => {
            ...
            // ⭐ [수정] sic-type (UNCOMMON | CURE 등) 라인을 완전히 삭제했습니다.
            el.innerHTML = `
                <div class="sic-top">
    ...
    openTavern() {
        ...
        pool.forEach((entry) => {
            ...
            const jobColors = { 
                'WARRIOR': '#8b0000', 'KNIGHT': '#2c3e50', 'MARTIAL ARTIST': '#b8860b', 'ROGUE': '#2d1e17', 
                'ARCHER': '#2e7d32', 'SORCERER': '#4a148c', 'CLERIC': '#c5a059', 'BARD': '#880e4f', 
                'DANCER': '#c2185b', 'ALCHEMIST': '#006064'
            };
            const accentColor = jobColors[entry.key] || '#5d4037'; // 동적 직업 색상은 인라인 유지 필요
    ...
    refreshTavern() {
        ...
        const HERO_CLASSES = [
            'WARRIOR', 'KNIGHT', 'MARTIAL ARTIST', 'ROGUE', 
    ...
    showItemTooltip(e, itemId) {
        ...
        if (tooltip) {
            const gradeClassMap = {
                'COMMON': 'grade-common', 'UNCOMMON': 'grade-uncommon',
                'RARE': 'grade-rare', 'EPIC': 'grade-epic', 'LEGENDARY': 'grade-legendary'
            };
            const gradeClass = gradeClassMap[item.grade] || 'grade-common';
                ...
                if (hero) {
                    ...
                    if (equippedId) {
                        ...
                        if (equippedItem) {
                            ...
                            if (diff > 0) diffHtml = ` <span class="tt-diff-up">(▲${diff})</span>`;
                            else if (diff < 0) diffHtml = ` <span class="tt-diff-down">(▼${Math.abs(diff)})</span>`;
                        }
                    } else if (slot) {
                        diffHtml = ` <span class="tt-diff-up">(NEW)</span>`;
                    }
                }
            }
            ...
            if (item.type === 'WEAPON') {
                ...
                if (hands === 2) typeText += ` <span class="tt-txt-2h">(2H)</span>`;
                else typeText += ` <span class="tt-txt-1h">(1H)</span>`;
            }

            let statHtml = "";
            if (item.type === 'WEAPON') {
                statHtml += `<div class="tt-stat-atk">⚔️ 공격력: ${item.val}${diffHtml}</div>`;
            } else if (['BODY', 'HEAD', 'SHIELD', 'LEGS'].includes(item.type)) {
                statHtml += `<div class="tt-stat-def">🛡️ 방어력: ${item.val}${diffHtml}</div>`;
            }

            // ⭐ [버그 수정] 소비 아이템(포션/재료)은 내부 시스템 변수(paralysis +0 등)를 화면에 노출하지 않고 숨깁니다.
            ...
            let skillHtml = "";
            // ⭐ [버그 수정] 아이템에 특수 스킬이 없을 경우 ("-" 표기) "효과: -" 가 뜨지 않도록 숨깁니다.
            ...
            if (item.jobs && item.jobs.length > 0) {
                jobHtml = `<div class="tt-job-req">가능 직업: ${item.jobs.join(', ')}</div>`;
            }
            ...
            tooltip.innerHTML = `
                <div class="tt-container">
                    ...
                    <div class="tt-price-box" style="margin-top: 10px;">
