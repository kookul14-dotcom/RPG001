...
export class GameRenderer {
    ...
    constructor(canvas, hexGrid, battleSystem) {
        ...
        this.grid = hexGrid;
        this.battle = battleSystem; // 주입된 의존성을 저장
    ...
    drawUnit(u, cam, isDead) {
        ...
        if (u.curHp <= 0 && u.isIncapacitated) {
            ctx.save();
            const textY = drawY - (40 * this.grid.scale); // 캐릭터 머리 위쪽 좌표
            const deathText = `⏳ ${u.deathTimer || 0}`;
            
            ctx.font = `bold ${16 * this.grid.scale}px sans-serif`; 
            const textWidth = ctx.measureText(deathText).width;
            
            // 텍스트가 잘 보이도록 반투명 검은색 배경 박스 깔기
            ctx.fillStyle = "rgba(0, 0, 0, 0.7)";
            ctx.fillRect(drawX - textWidth/2 - 6, textY - 12 * this.grid.scale, textWidth + 12, 24 * this.grid.scale);
            
            // 노란색 카운트다운 텍스트 렌더링
            ctx.fillStyle = "#ffdd00"; 
            ctx.textAlign = "center"; 
            ctx.textBaseline = "middle";
            ctx.fillText(deathText, drawX, textY);   
            ctx.restore();
        }
