...
export const CLASS_DATA = {
    // --- 플레이어 영웅 (10종) ---
    // AtkType 삭제됨. 각 스킬의 main.dmgType 확인
    'WARRIOR': { name: '전사', element: 'EARTH', level: 1, xp: 0, maxXp: 100, hp: 300, mp: 50, spd: 9, mov: 3, rng: 1, str: 25, int: 5, vit: 20, agi: 10, dex: 10, vol: 15, luk: 10, def: 15, icon: '⚔️', skills: [
        {id:'W1', name:'강타', mp:10, rng:1, cool:0, main:{type:'DMG', target:'ENEMY_SINGLE', mult:1.5, area:0, dmgType:'PHYS'}, sub:null},
        {id:'W2', name:'회전베기', mp:25, rng:1, cool:3, main:{type:'DMG', target:'AREA_ENEMY', mult:1.2, area:1, dmgType:'PHYS'}, sub:null},
        {id:'W3', name:'전투의함성', mp:40, rng:0, cool:5, main:{type:'ATK_UP', target:'SELF', mult:1.5, area:0, dmgType:'PHYS'}, sub:{type:'ATK_UP', target:'ALLY_ALL', mult:1.2, area:99, duration:3}}
    ]},
    'KNIGHT': { name: '기사', element: 'LIGHT', level: 1, xp: 0, maxXp: 100, hp: 400, mp: 60, spd: 7, mov: 3, rng: 1, str: 20, int: 8, vit: 30, agi: 5, dex: 5, vol: 5, luk: 10, def: 25, icon: '🛡️', skills: [
        {id:'K1', name:'방패치기', mp:15, rng:1, cool:2, main:{type:'DMG', target:'ENEMY_SINGLE', mult:1.2, area:0, dmgType:'PHYS'}, sub:{type:'STUN', target:'ENEMY_SINGLE', mult:0, area:0, duration:1}},
        {id:'K2', name:'수호의맹세', mp:30, rng:0, cool:4, main:{type:'DEF_UP', target:'SELF', mult:2, area:0, dmgType:'PHYS'}, sub:{type:'TAUNT', target:'AREA_ENEMY', mult:0, area:5, duration:3}},
        {id:'K3', name:'희생의방패', mp:60, rng:0, cool:8, main:{type:'SHLD', target:'ALLY_ALL', mult:3.0, area:99, dmgType:'PHYS'}, sub:{type:'DEF_UP', target:'ALLY_ALL', mult:1.5, area:99, duration:2}}
    ]},
    'MONK': { name: '몽크', element: 'WIND', level: 1, xp: 0, maxXp: 100, hp: 250, mp: 80, spd: 14, mov: 5, rng: 1, str: 22, int: 10, vit: 15, agi: 25, dex: 15, vol: 20, luk: 15, def: 5, icon: '👊', skills: [
        {id:'Mo1', name:'연타', mp:10, rng:1, cool:0, main:{type:'DMG', target:'ENEMY_SINGLE', mult:0.8, area:0, dmgType:'PHYS'}, sub:{type:'DMG', target:'ENEMY_SINGLE', mult:0.8, area:0, duration:0}},
        {id:'Mo2', name:'명상', mp:20, rng:0, cool:3, main:{type:'HEAL', target:'SELF', mult:0.4, area:0, dmgType:'PHYS'}, sub:{type:'PURIFY', target:'SELF', mult:0, area:0, duration:0}},
        {id:'Mo3', name:'비연각', mp:40, rng:3, cool:4, main:{type:'DMG', target:'ENEMY_SINGLE', mult:1.8, area:0, dmgType:'PHYS'}, sub:{type:'NUCKBACK', target:'ENEMY_SINGLE', mult:0, area:0, duration:0}}
    ]},
    'ROGUE': { name: '도적', element: 'DARK', level: 1, xp: 0, maxXp: 100, hp: 220, mp: 100, spd: 15, mov: 4, rng: 1, str: 20, int: 10, vit: 12, agi: 20, dex: 30, vol: 20, luk: 25, def: 8, icon: '🗡️', skills: [
        {id:'R1', name:'급소찌르기', mp:15, rng:1, cool:0, main:{type:'DMG', target:'ENEMY_SINGLE', mult:1.8, area:0, dmgType:'PHYS'}, sub:{type:'CRIT_UP', target:'SELF', mult:1.3, area:0, duration:2}},
        {id:'R2', name:'독바르기', mp:25, rng:1, cool:3, main:{type:'DMG', target:'ENEMY_SINGLE', mult:0.5, area:0, dmgType:'PHYS'}, sub:{type:'POISON', target:'ENEMY_SINGLE', mult:0.3, area:0, duration:4}},
        {id:'R3', name:'연막탄', mp:50, rng:3, cool:6, main:{type:'BLIND', target:'AREA_ENEMY', mult:0, area:2, dmgType:'PHYS'}, sub:{type:'SPD_DOWN', target:'AREA_ENEMY', mult:0.5, area:2, duration:3}}
    ]},
    'ARCHER': { name: '궁수', element: 'WIND', level: 1, xp: 0, maxXp: 100, hp: 200, mp: 80, spd: 12, mov: 3, rng: 6, str: 24, int: 8, vit: 10, agi: 18, dex: 28, vol: 15, luk: 15, def: 5, icon: '🏹', skills: [
        {id:'A1', name:'정밀사격', mp:15, rng:6, cool:0, main:{type:'DMG', target:'ENEMY_SINGLE', mult:1.4, area:0, dmgType:'PHYS'}, sub:{type:'CRIT_UP', target:'SELF', mult:1.2, area:0, duration:1}},
        {id:'A2', name:'화살비', mp:35, rng:5, cool:4, main:{type:'DMG', target:'AREA_ENEMY', mult:1.2, area:1, dmgType:'PHYS'}, sub:null},
        {id:'A3', name:'매의눈', mp:50, rng:0, cool:6, main:{type:'ATK_UP', target:'SELF', mult:1.5, area:0, dmgType:'PHYS'}, sub:{type:'rng', target:'SELF', mult:2, area:0, duration:3}}
    ]},
    'SORCERER': { name: '마법사', element: 'FIRE', level: 1, xp: 0, maxXp: 100, hp: 160, mp: 250, spd: 10, mov: 2, rng: 4, str: 5, int: 35, vit: 10, agi: 10, dex: 12, vol: 25, luk: 15, def: 5, icon: '🔮', skills: [
        {id:'S1', name:'화염구', mp:25, rng:5, cool:0, main:{type:'DMG', target:'ENEMY_SINGLE', mult:2, area:0, dmgType:'MAG'}, sub:null},
        {id:'S2', name:'낙뢰', mp:40, rng:5, cool:3, main:{type:'DMG', target:'ENEMY_SINGLE', mult:2.5, area:0, dmgType:'MAG'}, sub:{type:'SHOCK', target:'ENEMY_SINGLE', mult:0, area:0, duration:2}},
        {id:'S3', name:'눈보라', mp:80, rng:6, cool:8, main:{type:'DMG', target:'AREA_ENEMY', mult:1.5, area:2, dmgType:'MAG'}, sub:{type:'SPD_DOWN', target:'AREA_ENEMY', mult:0.5, area:2, duration:3}}
    ]},
    'CLERIC': { name: '성직자', element: 'LIGHT', level: 1, xp: 0, maxXp: 100, hp: 240, mp: 200, spd: 9, mov: 2, rng: 2, str: 10, int: 25, vit: 20, agi: 8, dex: 8, vol: 10, luk: 15, def: 12, icon: '✝️', skills: [
        {id:'C1', name:'치유의빛', mp:20, rng:3, cool:0, main:{type:'HEAL', target:'ALLY_SINGLE', mult:2, area:0, dmgType:'MAG'}, sub:null},
        {id:'C2', name:'정화', mp:30, rng:3, cool:2, main:{type:'PURIFY', target:'ALLY_SINGLE', mult:0, area:0, dmgType:'MAG'}, sub:{type:'HEAL', target:'ALLY_SINGLE', mult:1, area:0, duration:0}},
        {id:'C3', name:'성역', mp:100, rng:0, cool:10, main:{type:'HEAL', target:'ALLY_ALL', mult:1.5, area:99, dmgType:'MAG'}, sub:{type:'DEF_UP', target:'ALLY_ALL', mult:1.3, area:99, duration:2}}
    ]},
    'BARD': { name: '음유시인', element: 'WIND', level: 1, xp: 0, maxXp: 100, hp: 220, mp: 180, spd: 11, mov: 3, rng: 3, str: 8, int: 22, vit: 18, agi: 12, dex: 15, vol: 10, luk: 20, def: 8, icon: '🎻', skills: [
        {id:'Ba1', name:'불협화음', mp:20, rng:3, cool:0, main:{type:'DMG', target:'ENEMY_SINGLE', mult:1.2, area:0, dmgType:'MAG'}, sub:{type:'CONFUSE', target:'ENEMY_SINGLE', mult:0, area:0, duration:1}},
        {id:'Ba2', name:'용기의노래', mp:40, rng:0, cool:4, main:{type:'ATK_UP', target:'ALLY_ALL', mult:1.3, area:99, dmgType:'MAG'}, sub:null},
        {id:'Ba3', name:'평온의연주', mp:60, rng:0, cool:6, main:{type:'HEAL', target:'ALLY_ALL', mult:0, area:99, dmgType:'MAG'}, sub:{type:'PURIFY', target:'ALLY_ALL', mult:0, area:99, duration:0}}
    ]},
    'DANCER': { name: '무희', element: 'FIRE', level: 1, xp: 0, maxXp: 100, hp: 200, mp: 150, spd: 16, mov: 4, rng: 1, str: 10, int: 15, vit: 15, agi: 30, dex: 10, vol: 20, luk: 20, def: 8, icon: '💃', skills: [
        {id:'Da1', name:'매혹의춤', mp:25, rng:2, cool:3, main:{type:'DMG', target:'ENEMY_SINGLE', mult:0.5, area:0, dmgType:'PHYS'}, sub:{type:'SLEEP', target:'ENEMY_SINGLE', mult:0, area:0, duration:2}},
        {id:'Da2', name:'격정의춤', mp:40, rng:0, cool:5, main:{type:'SPD_UP', target:'ALLY_ALL', mult:1.3, area:99, dmgType:'PHYS'}, sub:{type:'EVA_UP', target:'ALLY_ALL', mult:1.3, area:99, duration:3}},
        {id:'Da3', name:'앙코르', mp:80, rng:2, cool:8, main:{type:'MP_HEAL', target:'ALLY_SINGLE', mult:0, area:0, dmgType:'PHYS'}, sub:{type:'cool_down', target:'ALLY_SINGLE', mult:3, area:0, duration:0}}
    ]},
    'ALCHEMIST': { name: '연금술사', element: 'WATER', level: 1, xp: 0, maxXp: 100, hp: 220, mp: 180, spd: 12, mov: 3, rng: 4, str: 10, int: 25, vit: 15, agi: 15, dex: 25, vol: 15, luk: 15, def: 10, icon: '⚗️', skills: [
        {id:'Al1', name:'포션투척', mp:15, rng:4, cool:0, main:{type:'HEAL', target:'ALLY_SINGLE', mult:1.5, area:0, dmgType:'MAG'}, sub:null},
        {id:'Al2', name:'산성폭탄', mp:30, rng:4, cool:3, main:{type:'DMG', target:'AREA_ENEMY', mult:1.2, area:1, dmgType:'MAG'}, sub:{type:'DEF_DOWN', target:'AREA_ENEMY', mult:0.7, area:1, duration:3}},
        {id:'Al3', name:'황금연성', mp:100, rng:4, cool:10, main:{type:'DMG', target:'ENEMY_SINGLE', mult:3, area:0, dmgType:'MAG'}, sub:{type:'STUN', target:'ENEMY_SINGLE', mult:0, area:0, duration:2}}
    ]},
