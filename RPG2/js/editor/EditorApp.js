...
export const EDITOR_PALETTE = {
    ...
    'GRASS_01':   { name: '풀밭 A', color: '#4caf50', side: '#2e7d32', cost: 1, defaultH: 0 },
    'GRASS_02':   { name: '풀밭 B', color: '#388e3c', side: '#1b5e20', cost: 1, defaultH: 0 },
    'GRASS_03':   { name: '건조한 풀', color: '#8bc34a', side: '#558b2f', cost: 1, defaultH: 0 },
    'FLOWER_01':  { name: '꽃밭',   color: '#a5d6a7', side: '#66bb6a', cost: 1, defaultH: 0 },
    'FOREST_01':  { name: '숲 바닥', color: '#1b5e20', side: '#0d3b10', cost: 2, defaultH: 0 },
    'BUSH_01':    { name: '덤불',    color: '#558b2f', side: '#33691e', cost: 2, defaultH: 0 },
    'DIRT_01':    { name: '흙길 A', color: '#795548', side: '#4e342e', cost: 1, defaultH: 0 },
    'DIRT_02':    { name: '흙길 B', color: '#5d4037', side: '#3e2723', cost: 1, defaultH: 0 },
    'DIRT_MUD':   { name: '진흙',   color: '#3e2723', side: '#261815', cost: 2, defaultH: 0 },
    'SAND_01':    { name: '모래 A', color: '#fdd835', side: '#f9a825', cost: 2, defaultH: 0 },
    'SAND_02':    { name: '고운 모래', color: '#fff59d', side: '#fbc02d', cost: 2, defaultH: 0 },
    'GRAVEL':     { name: '자갈',   color: '#9e9e9e', side: '#616161', cost: 1, defaultH: 0 },
    'PAVED':      { name: '포장도로', color: '#90a4ae', side: '#546e7a', cost: 1, defaultH: 0 },
    'WASTELAND':  { name: '황무지', color: '#8d6e63', side: '#5d4037', cost: 1, defaultH: 0 },
    'STONE_01':   { name: '암석',   color: '#757575', side: '#424242', cost: 1, defaultH: 0 },
    'ROCK_DARK':  { name: '검은 바위', color: '#424242', side: '#212121', cost: 1, defaultH: 1 },
    'MT_BASE':    { name: '산기슭', color: '#6d4c41', side: '#3e2723', cost: 2, defaultH: 1 },
    'CLIFF_01':   { name: '절벽',   color: '#8d6e63', side: '#5d4037', cost: 99, defaultH: 2 },
    'CAVE_FLR':   { name: '동굴바닥', color: '#37474f', side: '#263238', cost: 1, defaultH: -1 },
    'WATER_S':    { name: '얕은 물', color: '#4fc3f7', side: '#0288d1', cost: 3, defaultH: -0.5 },
    'WATER_D':    { name: '깊은 물', color: '#0288d1', side: '#01579b', cost: 99, defaultH: -1 },
    'SWAMP':      { name: '늪지대', color: '#558b2f', side: '#33691e', cost: 3, defaultH: -0.2 },
    'ICE_01':     { name: '빙판',   color: '#b3e5fc', side: '#81d4fa', cost: 1, defaultH: 0 },
    'SNOW_01':    { name: '눈밭',   color: '#e1f5fe', side: '#b3e5fc', cost: 2, defaultH: 0.2 },
    'LAVA_01':    { name: '용암',   color: '#ff5722', side: '#bf360c', cost: 99, defaultH: -1 },
    'LAVA_HOT':   { name: '마그마', color: '#ffccbc', side: '#ff5722', cost: 99, defaultH: -0.5 },
    'POISON_L':   { name: '독지대', color: '#9c27b0', side: '#4a148c', cost: 2, defaultH: -0.5 },
    'CRYSTAL':    { name: '수정바닥', color: '#e040fb', side: '#7b1fa2', cost: 1, defaultH: 0 },
    'SPACE':      { name: '우주',   color: '#311b92', side: '#000000', cost: 99, defaultH: -5 },
    'WOOD_DK':    { name: '나무데크', color: '#d7ccc8', side: '#8d6e63', cost: 1, defaultH: 0.5 },
    'BRIDGE':     { name: '다리',   color: '#ffecb3', side: '#ff6f00', cost: 1, defaultH: 0.5 },
    'BRICK_R':    { name: '붉은 벽돌', color: '#d84315', side: '#bf360c', cost: 1, defaultH: 0 },
    'BRICK_G':    { name: '회색 벽돌', color: '#cfd8dc', side: '#78909c', cost: 1, defaultH: 0 },
    'TILE_CHK':   { name: '체크타일', color: '#eceff1', side: '#546e7a', cost: 1, defaultH: 0 },
    'WALL_BIG':   { name: '거대 벽', color: '#3e2723', side: '#1b0000', cost: 99, defaultH: 3 },
    'TREE_OAK':   { name: '참나무', color: '#2e7d32', side: '#1b5e20', cost: 99, defaultH: 2 }
};
...
export const BUILDING_PALETTE = {
    ...
    // 1. 스테이지별 상자
    'BOX_STAGE_1':     { name: '📦 초반 보급상자', icon: '📦', color: '#8d6e63' }, // 갈색 느낌
    'BOX_STAGE_2':     { name: '🎁 중반 탐험상자', icon: '🎁', color: '#26a69a' }, // 청록색 느낌
    'BOX_STAGE_3':     { name: '👑 후반 황금상자', icon: '👑', color: '#ffd700' }, // 금색
...
class EditorApp {
    ...
    renderViewport(vpName) {
        ...
        sorted.forEach(h => {
            const data = vp.grid.terrainMap.get(`${h.q},${h.r}`) || { key: 'GRASS_01', h: 0 };
            const terrain = EDITOR_PALETTE[data.key] || EDITOR_PALETTE['GRASS_01'];
            ...
            if (vpName === 'bottom' && this.showInfo && vp.grid.scale > 0.4) {
                // 히든이 있으면 텍스트가 겹치지 않게 조금 더 아래로 내림
    ...
    drawHidden(vp, x, y, hiddenData) {
        ...
        vp.ctx.textAlign = "center";
        vp.ctx.textBaseline = "middle";
        vp.ctx.globalAlpha = 0.6; // 반투명하게 표시 (숨겨진 요소)
        ...
        if (hiddenData.text) {
            ...
            vp.ctx.fillStyle = "#ff88ff"; // 핑크색으로 구분
    ...
    drawHexInfo(vp, x, y, terrain, data) {
        ...
        const finalCost = this.getDynamicCost(terrain, data.h || 0);
        const costText = finalCost >= 99 ? "X" : `${finalCost}`;
        const costColor = finalCost >= 99 ? '#ff0000' : (textColor === '#ffffff' ? '#aaaaff' : '#0000ff');
        ...
        ctx.fillText(`c:${costText}`, x + 10 * vp.grid.scale, infoY);
    }

    getDynamicCost(terrain, height) {
        if (terrain.cost >= 99) return 99;
        let cost = terrain.cost;
        if (terrain.name.includes('물') || terrain.name.includes('늪')) {
             if (height < 0) cost += Math.floor(Math.abs(height));
        } else {
            if (height > 0) cost += Math.floor(height * 0.5);
        }
        return Math.min(99, cost);
    }
        ...
        for (const [key, data] of Object.entries(source)) {
            ...
            item.onclick = () => {
                ...
                if (type === 'terrain') {
                    ...
                    // 저장된 값이 있으면 우선 사용, 없으면 데이터의 기본 text(프리셋) 사용
            ...
            if (this.brush.type === 'terrain') {
                if (this.brush.key === 'DEPLOY_ZONE') {
                    if (currentData) {
                        ...
                        newData = { key: 'GRASS_01', h: 0, deploy: true };
                    }
                } else {
                ...
                newData = {
                    key: currentData ? currentData.key : 'GRASS_01',
    ...
    loadStage(vpName, stageId, forceDefault = false) {
        ...
        if (json) {
            ...
            for(let r=0; r<12; r++) {
                for(let c=0; c<22; c++) {
                    const q = c - (r - (r & 1)) / 2;
                    vp.grid.terrainMap.set(`${q},${r}`, { key: 'GRASS_01', h: 0 });
                }
            }
        }
    }
    ...
    exportJson() {
        ...
        for (const [pos, val] of vp.grid.terrainMap.entries()) {
            ...
            if (val.hidden) {
                ...
                const pText = val.hidden.text || ''; // 사용자 입력값 또는 프리셋값
                const hKey = val.hidden.key;         // 에디터 팔레트 키
                ...
                // 2. 프리셋 (자동 매핑)
                // 만약 텍스트가 BOX_로 시작하거나, 팔레트 키가 프리셋 키들에 포함된다면 -> ITEM으로 처리
                    ...
                    type = 'ITEM';
                    // 텍스트가 있으면 텍스트(ID) 사용, 없으면 키에 따른 기본값 할당 (안전장치)
                    if (pText) {
                        obj.id = pText;
                    } else {
                        // 만약 텍스트가 비어있다면 키를 보고 매핑 (안전장치)
                        ...
                        else obj.id = 'BOX_STAGE_1'; // 최후의 기본값
                    }
                }
